...
class GithubReporter:
    ...
    def generate_detailed_report(self, all_stats: List[RepoStats]) -> None:
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            ...
            active_repos_sorted = sorted([s for s in all_stats if
                                          s.activity.is_active and "Empty repository with no files" not in s.scores.anomalies],
                                         key=lambda x: x.activity.last_commit_date, reverse=True)[:10]
            for i, stats in enumerate(active_repos_sorted, 1):
                f.write(
                    f"{i}. **{stats.name}** - Last commit: {stats.activity.last_commit_date.strftime('%Y-%m-%d')}\n")
            f.write("\n")
            ...
            for stats in all_stats:
                ...
                f.write(
                    f"- **Last Pushed:** {stats.last_pushed.strftime('%Y-%m-%d') if stats.last_pushed else 'Unknown'}\n")
                ...
                if stats.has_docs:
                    f.write(
                        f"  - **Documentation Size:** {stats.docs_size_category} ({stats.docs_files_count} files)\n")
                f.write(f"- **Has README:** {'✅ Yes' if stats.has_readme else '❌ No'}\n")
                if stats.has_readme:
                    f.write(
                        f"  - **README Quality:** {stats.readme_comprehensiveness} ({stats.readme_line_count} lines)\n")
                ...
                if stats.has_tests:
                    ...
                    if stats.quality.test_coverage_percentage is not None:
                        ...
                        f.write(
                            f"  - **Estimated Test Coverage:** {coverage_emoji} {stats.quality.test_coverage_percentage:.1f}% (estimated from file count)\n")
                ...
                f.write("### 📅 Activity\n")
                f.write(
                    f"- **Last Commit:** {stats.last_commit_date.strftime('%Y-%m-%d %H:%M:%S') if stats.last_commit_date else 'Unknown'}\n")
                ...
                if stats.project_structure:
                    ...
                    has_scripts = any(
                        d.lower() in ['script', 'scripts', 'tools', 'util', 'utils'] for d in stats.project_structure)
                    has_examples = any(
                        d.lower() in ['example', 'examples', 'demo', 'demos', 'sample', 'samples'] for d in
                        stats.project_structure)
    ...
    def generate_aggregated_report(self, all_stats: List[RepoStats]) -> None:
        ...
        avg_maintenance_score = sum(
            stats.maintenance_score for stats in non_empty_repos) / non_empty_count if non_empty_count > 0 else 0
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            ...
            if total_empty_repos > 0:
                f.write(
                    f"- **Empty Repositories:** {total_empty_repos:,} ({total_empty_repos / total_repos * 100:.1f}%)\n")
            ...
            if total_excluded_files > 0:
                excluded_percentage = (
                            total_excluded_files / all_files_including_excluded * 100) if all_files_including_excluded > 0 else 0
                f.write(
                    f"- **Files Excluded from Analysis:** {total_excluded_files:,} ({excluded_percentage:.1f}% of all files)\n")
            ...
            if non_empty_count > 0:
                f.write(
                    f"- **Repositories with Documentation:** {repos_with_docs} ({repos_with_docs / non_empty_count * 100:.1f}% of non-empty)\n")
                ...
                if docs_size_categories:
                    ...
                    for category, count in sorted(docs_size_categories.items(),
                                                  key=lambda x: ["None", "Small", "Intermediate", "Big"].index(x[0])):
                ...
                f.write(
                    f"- **Repositories with README:** {repos_with_readme} ({repos_with_readme / non_empty_count * 100:.1f}% of non-empty)\n")
                ...
                if readme_categories:
                    ...
                    for category, count in sorted(readme_categories.items(),
                                                  key=lambda x: ["None", "Small", "Good", "Comprehensive"].index(x[0])):
                ...
                f.write(
                    f"- **Repositories with Tests:** {repos_with_tests} ({repos_with_tests / non_empty_count * 100:.1f}% of non-empty)\n")

                # Package management stats
                repos_with_packages = sum(1 for s in non_empty_repos if s.has_packages)
                f.write(
                    f"- **Repositories with Package Management:** {repos_with_packages} ({repos_with_packages / non_empty_count * 100:.1f}% of non-empty)\n")

                # Deployment configuration stats
                repos_with_deployments = sum(1 for s in non_empty_repos if s.has_deployments)
                f.write(
                    f"- **Repositories with Deployment Configuration:** {repos_with_deployments} ({repos_with_deployments / non_empty_count * 100:.1f}% of non-empty)\n")

                # Release stats
                repos_with_releases = sum(1 for s in non_empty_repos if s.has_releases)
                f.write(
                    f"- **Repositories with Releases:** {repos_with_releases} ({repos_with_releases / non_empty_count * 100:.1f}% of non-empty)\n")
            ...
            if repos_with_coverage:
                avg_test_coverage = sum(s.quality.test_coverage_percentage for s in repos_with_coverage) / len(
                    repos_with_coverage)
                ...
                if repos_with_coverage_len > 0:
                    f.write(
                        f"  - **High Coverage (>70%):** {high_coverage} repos ({high_coverage / repos_with_coverage_len * 100:.1f}% of tested)\n")
                    f.write(
                        f"  - **Medium Coverage (30-70%):** {med_coverage} repos ({med_coverage / repos_with_coverage_len * 100:.1f}% of tested)\n")
                    f.write(
                        f"  - **Low Coverage (<30%):** {low_coverage} repos ({low_coverage / repos_with_coverage_len * 100:.1f}% of tested)\n")

            if non_empty_count > 0:
                f.write(
                    f"- **Active Repositories:** {active_repos} ({active_repos / non_empty_count * 100:.1f}% of non-empty)\n")
            else:
                f.write(f"- **Active Repositories:** 0 (0.0% of non-empty)\n")

            if total_repos > 0:
                f.write(
                    f"- **Repositories with License:** {len(license_counts)} ({len(license_counts) / total_repos * 100:.1f}% of total)\n")
            else:
            ...
            if non_empty_repos:
                primary_languages = Counter(
                    stats.primary_language for stats in non_empty_repos if stats.primary_language)
            ...
            if monorepos:
                if non_empty_count > 0:
                    ...
                    f.write(
                        f"- **Monorepos Detected:** {len(monorepos)} ({monorepo_percentage:.1f}% of non-empty repos)\n")
                else:
                    f.write(f"## 📦 Monorepo Analysis\n\n")
                    f.write(f"- **Monorepos Detected:** {len(monorepos)} (N/A% of non-empty repos)\n")

                f.write(
                    f"- **Average LOC in Monorepos:** {sum(s.total_loc for s in monorepos) / len(monorepos):,.0f}\n")
                f.write("\n")
            ...
            if non_empty_repos:
                ...
                if active_repos > 0:
                    f.write(
                        f"- **Average Monthly Commits per Active Repo:** {total_commits_last_month / active_repos:.1f} (active repos only)\n")
                else:
